...
{
    public class Form_BT_POWER_DAYDA : BaseDA
    {
        public override int Delete(FormM form, DFDictionary entity, ref string message)
        {
            ...
            Delete("delete from BT_POWER_DAY where PKID=@PKID", data.Select(a => new { PKID = a["PKID"] }).ToList());
            message = "删除成功".GetRes();
        ...
        {
            var count = 0;
            var sql = @"select * from BT_POWER_DAY where 1=1";
            var param = new
            {
