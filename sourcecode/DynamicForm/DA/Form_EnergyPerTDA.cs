...
{
    public class Form_EnergyPerTDA:BaseDA
    {
        public override int Query(FormM form, DFDictionary entity, DataGridVM vm, int start, int limit, ref string message)
        {
            var count = 0;
            var sql = @"exec sp_energypertquery @Line=@Line,@Category=@Category,@CollectTimeFrom=@CollectTimeFrom,@CollectTimeTo=@CollectTimeTo";
            var param = new
            {
                Line = entity["Line"],
