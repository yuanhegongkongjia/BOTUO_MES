...
{
    public class Form_SM_T_PROCESS_CY_LOGDA : BaseDA
    {
        public override int Delete(FormM form, DFDictionary entity, ref string message)
        {
            ...
            if (data == null)
            {
                ...
                Delete("delete from SM_T_PROCESS_CY_LOG where LogId=@LogId", data.Select(a => new { LogId = a["LogId"] }).ToList());
            message = "删除成功".GetRes();
        ...
        {
            var count = 0;
            var sql = @"select * from SM_T_PROCESS_CY_LOG where 1=1";
            var param = new
            {
