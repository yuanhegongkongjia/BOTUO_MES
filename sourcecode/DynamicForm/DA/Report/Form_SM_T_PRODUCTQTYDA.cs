...
{
    public class Form_SM_T_PRODUCTQTYDA : BaseDA
    {
        public override int Delete(FormM form, DFDictionary entity, ref string message)
        {
            ...
            Delete("delete from SM_T_PRODUCTQTY where PKId=@PKId", data.Select(a => new { PKId = a["PKId"] }).ToList());
            message = "删除成功".GetRes();
        ...
        {
            var count = 0;
            var sql = @"select * from SM_T_PRODUCTQTY where 1=1";
            var param = new
            {
