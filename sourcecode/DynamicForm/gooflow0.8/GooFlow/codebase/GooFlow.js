function currentDate() {
    ...
    return y + (m < 10 ? ('0' + m) : m) + (d < 10 ? ('0' + d) : d) + (hh < 10 ? ('0' + hh) : hh) + (mm < 10 ? ('0' + mm) : mm) + (ss < 100 ? ('00' + ss) : ss)
}
//定义一个区域图类：
function GooFlow(bgDiv, property) {
    ...
    if (this.$editable) {
        this.$workArea.on("click", { inthis: this }, function (e) {
            ...
            //This.$max++;
            This.addNode(This.$id + "_node_" + currentDate(), { name: "node_" + currentDate(), left: X, top: Y, type: e.data.inthis.$nowType, width: 135, height: 50 });
        });
        //划线或改线时用的绑定
...
GooFlow.prototype = {
    ...
    //2015-07-28 TXF
    addNode: function (id, json, disabled) {
        if (!disabled)
        ...
        if (json.type != "start" && json.type != "end") {
            if (!json.width || json.width < 135) json.width = 135;
            if (!json.height || json.height < 50) json.height = 50;
    ...
    initWorkForNode: function () {
        ...
        this.$workArea.delegate(".GooFlow_item", "mouseup", { inthis: this }, function (e) {
            ...
            if (lineStart && !This.$mpTo.data("p")) {
                ...
                //This.$max++;
                if (lineStart) This.addLine(This.$id + "_line_" + currentDate(), { from: lineStart.id, to: this.id, name: "", type: "sl" });
            }
            else {
                if (lineStart) {
    ...
    loadData: function (data) {
        ...
        for (var i in data.nodes)
            this.addNode(i, data.nodes[i], true);//2015-07-28 加载不要触发onItemAdd事件
        for (var j in data.lines)
            this.addLine(j, data.lines[j], true);
        for (var k in data.areas)
            this.addArea(k, data.areas[k], true);
        this.$editable = t;
        this.$deletedItem = {};
    },
    //用AJAX方式，远程读取一组数据
    ...
    loadDataAjax: function (param) {
        ...
        $.ajax({
            ...
            success: function (obj) {
                if (param.dataFilter) param.dataFilter(obj.flowData, "json");
                This.loadData($.parseJSON(obj.flowData));
                if (param.success) param.success(obj);
            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                if (param.error) param.error(textStatus, errorThrown);
            }
        })
    },
    ...
    //增加一条线
    addLine: function (id, json, disabled) {
        if (!disabled)
        ...
        // 添加额外的参数 todd zhuang
        this.$lineData[id].strategy = json.strategy;
        this.$lineData[id].scripttype = json.scripttype;
        this.$lineData[id].ModelId = json.ModelId;
    ...
    setAreaColor: function (id, color) {
        if (!this.$areaData[id]) return;
        if (this.onAreaSetColor != null && !this.onAreaSetColor(id, color)) return;
        if (this.$undoStack) {
    ...
    addArea: function (id, json, disabled) {
        if (!disabled)
