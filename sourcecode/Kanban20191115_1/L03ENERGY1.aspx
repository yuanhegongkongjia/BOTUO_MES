<%@ Page Title="" Language="C#" MasterPageFile="~/KanbanSite.Master" AutoEventWireup="true" CodeBehind="L03ENERGY1.aspx.cs" Inherits="Kanban.L03ENERGY1" %>
...
<div class="page-header">
        <div class="logo">
            ...
            <span style="font-size: 28px; color: white;" onclick="requestFullScreen();">三号线炉压/能源看板</span>
    ...
    <script>
        ...
        function prePage() {
            window.location.href = "L03MFUA.aspx";
        }

        function nextPage() {
            window.location.href = "L03LW.aspx";
        }
        ...
        function requestLYData() {
            $.getJSON('../Handler/ProcessHandler.ashx?action=l03ly', function (data) {
                if (data.hasError == true) {
                    ...
                    //if (sd == 0) {
                    for (i in d) {
                        series.push([new Date(d[i].time).getTime(), d[i].value]);
                            //lyChart.addSeries({
                            //    name: d[i].name,
                            //    data: d[i].data
                            //});
        ...
        function requestpowerData() {
            $.getJSON('../Handler/EnergyHandler.ashx?action=getpower', { 'Line': 'L03' }, function (data) {
                if (data.hasError == true) {
                    alert(data.error);
                    return;
                }
        ...
        function requestZhengQiData() {
            ...
            $.getJSON('../Handler/EnergyHandler.ashx?action=gettzhengqi', { 'Line': 'L03' }, function (data) {
                if (data.hasError == true) {
                    alert(data.error);
                    return;
                }
        ...
        function requestTianRanQiData() {
            $.getJSON('../Handler/EnergyHandler.ashx?action=getTianRanQi', { 'Line': 'L03' }, function (data) {
                if (data.hasError == true) {
                    alert(data.error);
                    return;
                }
